...
def report_email(year: int | None = None, month: int | None = None,
                 email: str | None = None):
    """Build the report and email the .pptx via the user's local Outlook."""
    _require_report()
    c = client()
    y, m = _report_period(year, month)
    try:
    ...
    import os
    import tempfile
    label = report_module.month_label(y, m)
    path = os.path.join(tempfile.gettempdir(), f"monthly-report-{y}-{m:02d}.pptx")
    with open(path, "wb") as fh:
        fh.write(data)

    to = config.REPORT_EMAIL
    if not to:
        return {"sent": False, "to": "", "error": "No recipient email configured."}
    display = config.EMAIL_MODE == "display"
    try:
        from .emailer import send_via_outlook
        sender = send_via_outlook(
            to,
            f"Monthly Report - {label} - {owner}",
            f"Attached is the monthly Jira report for {owner} ({label}).\n\n"
            "Generated by Jira Manager.",
            path,
            display=display,
        )
        return {"sent": not display, "displayed": display, "to": to,
                "from": sender}
    except Exception as exc:  # noqa: BLE001 - report back, don't 500
